...
def main():
    ...
    parser.add_argument("--save_every", type=int, default=1,
                        help="每隔几个 epoch 保存一次 checkpoint")
    ...
    if args.variant == "baseline_frozen":
        model = BERTBaselineFrozen(model_path=args.model_path)
    elif args.variant == "baseline_qkv":
        model = BERTBaselineQKV(model_path=args.model_path)
    elif args.variant == "qkv_only":
        model = BERTQKVOnly(model_path=args.model_path, rank=51)
    ...
    variant_labels = {
        ...
        "baseline_qkv":    "Baseline-QKV  (上界, {:,}M QKV)".format(trainable//1000000),
        "qkv_only":        "QKV-Only      (低秩对照, {:,}M)".format(trainable//1000000),
        "tucker":          "Tucker QKV    (你的方案, {:,}M)".format(trainable//1000000),
    }
    ...
    start_epoch = 1
    results = {"variant": args.variant, "epochs": []}
    best_ppl = float("inf")

    ckpt_path = os.path.join(args.output_dir, f"bert_{args.variant}_checkpoint.pt")
    ...
    results_path = os.path.join(args.output_dir, f"bert_{args.variant}_results.json")
    ...
    print(f"下次运行: python train_bert.py --variant {args.variant} --data_file <file> 即可自动续训")
